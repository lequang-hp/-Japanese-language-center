            ...
            <div class="col-lg-9" >
                <div class="contant" style="width: 95%;height: 100%;padding: 10px;background-color: white;">
                    <form action="NopBai" method="post">
                    	<p style="font-weight: bold;color:red;">Chọn đáp án đúng nhất trong các đáp án 1,2,3,4</p>
	                   	<c:forEach items="${list_quest}" var="r">
	                    	<b>${r.id}</b><span>: </span><span style="font-family: Times;"><b>${r.quest}</b></span><br/>
	                    	<input type = "radio"  name = "ans[${r.id}]" value = "1">${r.ans1}<br/>
							<input type = "radio"  name = "ans[${r.id}]" value = "2">${r.ans2}<br/>
							<input type = "radio"  name = "ans[${r.id}]" value = "3">${r.ans3}<br/>
							<input type = "radio"  name = "ans[${r.id}]" value = "4">${r.ans4}<br/><br/>
	                    </c:forEach>
                   		<button type="submit" class="btn btn-outline-primary">Nộp bài</button>
                    </form>
